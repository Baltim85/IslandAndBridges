...
public class CalculateGrid {
	...
	/**
	 * Die x-Koordinate, die als Referenzpunkt für das Raster verwendet wird.
	 */
	private int zeroX;

	/**
	 * Die y-Koordinate, die als Referenzpunkt für das Raster verwendet wird.
	 */
	private int zeroY;

	/**
	 * Der Abstand zwischen den Rasterlinien in horizontaler Richtung (x-Richtung).
	 */
	private int xDistance;

	/**
	 * Der Abstand zwischen den Rasterlinien in vertikaler Richtung (y-Richtung).
	 */
	...
	private void calculateLeftZero() {
	    if (gridHeight % 2 != 0) {
	        ...
	        int gridZeroLeftEven = getCenterY() - (xDistance / 2);
	        for (int i = 0; i < gridHeight - 2; i++) {
	            if (i % 2 == 0) {
	                // Die Nullposition für gerade Linien wird nach links angepasst
	                gridZeroLeftEven -= xDistance;
	            }
	        }
	        // Die berechnete Nullposition für x setzen
	...
	private void calculateUpperZero() {
	    if (gridWidth % 2 != 0) {
	        ...
	        int gridWidthDownPos = getCenterX() - (yDistance / 2);
	        for (int i = 0; i < gridWidth - 2; i++) {
	            if (i % 2 == 0) {
	                // Die Nullposition für gerade Linien wird nach oben angepasst
	                gridWidthDownPos -= yDistance;
	            }
	        }
	        // Die berechnete Nullposition für y setzen
