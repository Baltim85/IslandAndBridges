...
/**
 * Die Klasse `NorthBridges` ist ein Teil des Bridge-Bauprozesses im Brückenrätselspiel und ist zuständig für das Erstellen von Brücken in nördlicher
 *  Richtung von einer ausgewählten Insel. Hier werden die Spiellogik und die Berechnungen für die Brückenerstellung und -überprüfung durchgeführt.
 *
 * Attribute:
 ...
 *
 * Konstruktor:
 * - `NorthBridges(GameData data, LookForAnotherBridge lookForAnotherBridge, UpdateBridgeCounter updateBridgeCounter)`: 
 * Erzeugt einen `NorthBridges`-Controller mit den benötigten Spiel- und Brückendaten.
 *
 * Methoden:
 * - `createNorthBridge(boolean leftButton, int selectedID)`: Erstellt eine Brücke in nördlicher Richtung von der ausgewählten Insel, basierend auf Benutzereingaben und Spielregeln.
 ...
 * - `delta(int y1, int y2)`: Aktualisiert die Zeichnungskoordinaten für die Linie basierend auf der ausgewählten Richtung und den y-Koordinaten.
 * - Weitere Getter und Setter-Methoden für die Attribute der Klasse.
 */
public class NorthBridges implements CreateNewBridges {
	...
	/**
	 * Initialisiert eine Instanz der NorthBridges-Klasse mit den angegebenen Spieldaten.
	 *
	 * @param data Die Spieldaten, in denen die Brückeninformationen verwaltet werden.
	 */
	public NorthBridges(GameData data) {
		this.data = data;
	}
	...
	/**
	 * Erstellt eine Brücke in nördlicher Richtung von der ausgewählten Insel.
	 *
	...
	public void createNorthBridge(boolean leftButton, int selectedID) {
	    ...
	    for (int i = getSelectedY() - 1; i > -1; i--) {
	        for (Island island : data.getDeepCopy()) {
	            ...
	            if(CreateNewBridges.anotherBridge(Directions.NORTH, getSelectedX(), 0, i, data))
	                return;
	...
	public void handleBridgeCreation(boolean leftButton) {
	    ...
	    if (data.getBridgesList().isEmpty() && leftButton) {
	        ...
	        // Setze die ausgewählte ID in den Bridge-Controller.
	        //data.getCreateBridges().setSelectedID(getSelectedID());
	        // Überprüfe, ob das Hinzufügen einer Brücke möglich ist.
	        
	        if (data.getAddAndRemoveBridges().removeAndAddDoubleBridges(getId(), leftButton, getCenterX(), getDrawLineY(), getCenterX(), getDrawLineY2(), Directions.NORTH, getSelectedID())) {
