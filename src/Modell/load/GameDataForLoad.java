...
/**
 * Die Klasse "GameData" repräsentiert die Spielfelddaten, die aus einer geladenen Datei extrahiert wurden. Sie enthält 
 * Informationen über die Spielfeldgröße, die Anzahl der Inseln, die Positionen der Inseln und die Brücken zwischen den Inseln.

 * Attribute:
 * - width: Die Breite des Spielfelds, die die Anzahl der Spalten auf dem Spielfeld repräsentiert.
 * - height: Die Höhe des Spielfelds, die die Anzahl der Zeilen auf dem Spielfeld repräsentiert.
 * - islands: Die Anzahl der Inseln im Spiel, die auf dem Spielfeld vorhanden sind.
 * - islandsList: Eine Liste von Koordinaten (Spalten und Zeilen) der Inseln auf dem Spielfeld. Jedes Element in der Liste ist 
 * ein int-Array, das die Koordinaten der Insel repräsentiert.
 * - bridgeList: Eine Liste von Brücken auf dem Spielfeld, wobei jede Brücke durch ihre Start- und Endkoordinaten repräsentiert 
 * wird. Jedes Element in der Liste ist ein int-Array, das die Start- und Endkoordinaten der Brücke enthält.

 * Methoden:
 * - isValid(): Diese Methode überprüft die Gültigkeit der geladenen Spielfelddaten, einschließlich der Spielfeldgröße, der 
 * Anzahl der Inseln und der Informationen über Inseln und Brücken. Sie gibt true zurück, wenn die Daten gültig sind, andernfalls 
 * false.

 * - isIslandValid(): Diese private Methode überprüft, ob die Informationen über die Inseln auf dem Spielfeld gültig sind. 
 * Sie prüft, ob die Inselkoordinaten innerhalb des Spielfelds liegen und ob die Anzahl der Brücken pro Insel gültig ist.

 * - isBridgeValid(): Diese private Methode überprüft, ob die Informationen über die Brücken auf dem Spielfeld gültig sind. 
 * Sie prüft, ob die IDs der verbundenen Inseln gültig sind.
 ...
 * - getIslands(): Diese Methode gibt die Anzahl der Inseln im Spiel zurück.

 * - setIslands(int islands): Diese Methode legt die Anzahl der Inseln im Spiel fest.

 * - getIslandsList(): Diese Methode gibt die Liste von Koordinaten (Spalten und Zeilen) der Inseln auf dem Spielfeld zurück.

 * - setIslandsList(List<int[]> islandsList): Diese Methode legt die Liste von Koordinaten (Spalten und Zeilen) der Inseln auf 
 * dem Spielfeld fest.

 * - getBridgeList(): Diese Methode gibt die Liste von Brücken auf dem Spielfeld zurück, wobei jede Brücke durch ihre Start- 
 * und Endkoordinaten repräsentiert wird.

 * - setBridgeList(List<int[]> bridgeList): Diese Methode legt die Liste von Brücken auf dem Spielfeld fest.

 * Die Klasse "GameData" ist von zentraler Bedeutung, um die geladenen Spielfelddaten zu speichern und zu überprüfen, ob sie 
 * gültig sind. Sie bietet Methoden, um auf die Spielfelddaten zuzugreifen und sicherzustellen, dass das geladene Spiel korrekt 
 * wiederhergestellt werden kann.
 */
public class GameDataForLoad {
